...
class EsmSequenceTokenizer(PreTrainedTokenizerFast, EsmTokenizerBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # CHANGE THIS BLOCK:
        # Remove unk_token, cls_token, etc., from the super() call
        # because they are already inside the tokenizer_object.
        super().__init__(
            tokenizer_object=tokenizer,
            **kwargs, 
        )
    ...
    def _get_token(self, token_name: str) -> str:
        # 1. Hardcoded ESMC defaults - the most direct path
        mapping = {
            "pad_token": "<pad>",
            "cls_token": "<cls>",
            "mask_token": "<mask>",
        ...
        # 2. Try the mapping first
        target = mapping.get(token_name)
        
        # 3. Check every possible internal location for the Rust tokenizer
        # (HF changes these names between versions)
        internal_tokenizer = None
        for attr in ["tokenizer_object", "_tokenizer", "backend_tokenizer"]:
            if hasattr(self, attr):
                internal_tokenizer = getattr(self, attr)
                break
        
        # 4. If we found a tokenizer, verify the token exists
        if internal_tokenizer is not None:
            try:
                # If target is None (not in our map), use the original token_name
                check_str = target if target else token_name
                if internal_tokenizer.token_to_id(check_str) is not None:
                    return check_str
            except:
                pass

        # 5. Return the hardcoded default if all else fails
        if target:
            return target

        raise AttributeError(f"Could not find special token: {token_name}")